...
exports.ipoBuyStock = async (req, res) => {
  ...
  Promise.all([
    ...
    buyingUserDoc.update({
      ...
      totalAccountValue: firestoreRef.FieldValue.increment(
        numShares * stockData.currPoints - stockData.ipoPrice * numShares
      ),
    }),
  ...
  buyingUserDoc
    ...
    .update({
      ...
      stockName: stockData.stockName,
      avgBuyPrice:
        (buyerAvgBuyPrice * buyerSharesOwned + numShares * stockData.ipoPrice) /
...
exports.ipoSellStock = async (req, res) => {
  ...
  Promise.all([
    sellingUserDoc.update({
      ...
      totalAccountValue: firestoreRef.FieldValue.increment(
        numShares * (stockData.ipoPrice / 2) - numShares * stockData.currPoints
      ),
    }),
